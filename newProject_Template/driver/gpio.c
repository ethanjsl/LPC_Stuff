...
** file name:			gpio.c
**
** Descriptions:		Driver for the GPIO on the LPC1114.
**
** Author (Original):	NXP
**
** Author (changes):	Ethan Lyons, Fr. Sauter AG Basel				
**
** Date created:		03.09.2010
**
** Date edited:			18.12.2015
**
** Edited because:		Changes were made to use this as an example project.
**						"GPIOGetValue" was added.
**
******************************************************************************/
...
**
** Descriptions:		Read the Value of a Pin.
**
** parameters:			port num, bit position
**
** Returned value:		None
...
uint32_t GPIOGetValue (uint32_t portNum, uint32_t bitPosi )
{
	...
	{
	case PORT0:
		if (LPC_GPIO0->DATA &= (1<<bitPosi)){
			val=1;
		}
	case PORT1:
		if (LPC_GPIO1->DATA &= (1<<bitPosi)){
			val=1;
		}
		break;
	case PORT2:
		if (LPC_GPIO2->DATA &= (1<<bitPosi)){
			val=1;
		}
		break;
	case PORT3:
		if (LPC_GPIO3->DATA &= (1<<bitPosi)){
			val=1;
		}
		break;
	}
